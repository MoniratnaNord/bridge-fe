...
export default function TransferSuccessBaseModal({
	isOpen,
	onClose,
	fromAddress,
	toAddress,
			...
			<div className="relative w-full max-w-md rounded-2xl bg-slate-900 border border-slate-700 p-6 shadow-xl">
				...
				<div className="mt-6 space-y-3 text-sm">
					...
					<div className="flex justify-between">
						...
						<span className="text-white font-medium">{amountSent}</span>
					</div>

					<div className="flex justify-between">
						<span className="text-slate-400">Amount Received</span>
						<span className="text-green-400 font-medium">{amountReceived}</span>
					</div>
					{/* <div className="flex justify-between">
				...
				<div className="grid grid-cols-2 gap-2">
					<button
						onClick={() =>
							window.open(
								`${import.meta.env.VITE_BASE_EXPLORER_URL}/${polHash}`,
								"_blank",
							)
						}
						className="mt-6 w-full rounded-lg bg-orange-500 hover:bg-orange-600 py-2.5 text-white font-medium transition"
					>
						USDC txn on BASE Scan
					</button>
					<button
						onClick={() =>
							window.open(
								`${import.meta.env.VITE_BASE_EXPLORER_URL}/${xrpHash}`,
								"_blank",
							)
						}
						className="mt-6 w-full rounded-lg bg-orange-500 hover:bg-orange-600 py-2.5 text-white font-medium transition"
					>
						EURC TXN on BASE Scan
					</button>
				</div>
			</div>
		</div>
